...
# export XNAT_PdASS=${3}
# export XNAT_HOST=${4}
sessionID='SNIPR_E01028' #'SNIPR02_E02933' #${1}
...
function call_get_resourcefiles_metadata_saveascsv_args() {
  ...
  outputfiles_present=$(python software/download_with_session_ID.py "${call_download_files_in_a_resource_in_a_session_arguments[@]}")
...
while IFS=',' read -ra array; do
  ...
  while IFS=',' read -ra array1; do
    ...
    outputfiles_present=$(python utilities_atul.py "${call_gray2binary_arguments[@]}")
    ...
    outputfiles_present=$(python utilities_atul.py "${call_createh5file_arguments[@]}")
...
outputfiles_present=$(python utilities_atul.py "${call_vertical_extent_ventricle_arguments[@]}")
...
outputfiles_present=$(python utilities_atul.py "${call_copy_im_parameter_to_a_matrix_nifti_arguments[@]}")
...
outputfiles_present=$(python utilities_atul.py "${call_first_rotation_image_arguments[@]}")
...
outputfiles_present=$(python utilities_atul.py "${call_ventricle_boundingbox_each_slice_arguments[@]}")
...
outputfiles_present=$(python utilities_atul.py "${call_rotate_reverse_image_arguments[@]}")
  done \
