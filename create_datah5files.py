...
BET_GRAY_DIRECTORY='/storage1/fs1/dharr/Active/ATUL/PROJECTS/NWU/DATA/LVO/output_directoryAugust192021/OUTPUT_42_82/BET_OUTPUT'
BET_BINARY_DIRECTORY='/storage1/fs1/dharr/Active/ATUL/PROJECTS/NWU/DATA/LVO/output_directoryAugust192021/betmask'
template_T_OUTPUT_dir='/storage1/fs1/dharr/Active/ATUL/PROJECTS/NWU/DATA/LVO/output_directoryAugust192021/OUTPUT_42_82/TEMPLATE_TRANS_OUTPUT'
h5data_output_dir='/storage1/fs1/dharr/Active/ATUL/PROJECTS/DeepReg/demos/classical_mr_prostate_nonrigid/dataset'
for filename in glob.glob(os.path.join(template_T_OUTPUT_dir,'*_BET.nii.gz')):
    template_gray_file=filename.split('_BET')[0]+'.nii.gz'
#     print(template_gray_file)
    grayfilebase=os.path.basename(filename).split('scct_strippedResampled1')[1].split('_BET')[0]+'.nii.gz'
    grayfilename=os.path.join(BET_GRAY_DIRECTORY,grayfilebase)
#     print(grayfilename)
    betfile_base=os.path.basename(filename).split('scct_strippedResampled1')[1].split('_brain_f')[0]+'_bet.nii.gz'
    betfilename=os.path.join(BET_BINARY_DIRECTORY,betfile_base)
#     print(betfilename)
    if os.path.exists(template_gray_file) and os.path.exists(grayfilename) and os.path.exists(betfilename): # and os.path.exists(betfilename):
        print("ALL EXIST")
        createh5file(template_gray_file,grayfilename,filename,betfilename,output_dir=h5data_output_dir)
#         x=1
